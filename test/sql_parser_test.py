...
class DocumentObject(object):
    ...
    def __str__(self):
        return "name: %s, tag: %s, role: %s, num: %s" % (
            self.name,
            self.tag,
            self.role,
            self.num,
...
documentObjects = [wall, window, space, space001, space002, space003, norole]
...
def singleObjectSupplier(objectName):
    return [
        ...
        if documentObject.name == objectName
    ]
...
def shouldFilterNoneAttributes():
    result = executeStatement("Select * From document Where role IS NOT Null")

    assert result == [[wall], [window], [space], [space001], [space002], [space003]]


def shouldOnlyShowNoneAttributes():
    result = executeStatement("Select * From document Where role IS Null")

    assert result == [[norole]]


def selectWithLikeClause():
    result = executeStatement("Select * From document Where name Like 'Space%'")

    assert result == [[space], [space001], [space002], [space003]]

    result = executeStatement("Select * From document Where name Like 'Space0?2'")

    assert result == [[space002]]


def selectWithFunctionInWhereClause():
    result = executeStatement("Select * From document Where Lower(name) Like 'space%'")
...
def selectWithSumFunction():
    result = executeStatement("Select Sum(num) From document")

    assert result == [[21]]


def selectWithCountFunction():
    result = executeStatement("Select Count(*) From document Where role = 'space'")

    assert result == [[4]]


def selectWithMinFunction():
    result = executeStatement("Select Min(num) From document")

    assert result == [[1]]


def selectWithMaxFunction():
    result = executeStatement("Select Max(num) From document")

    assert result == [[6]]


def selectWithUnknownPropertyShouldIgnoreObject():
    result = executeStatement("Select * From document Where unkown = 'bedroom'")
...
def runGroupByClause():
    result = executeStatement("Select role, count(*) From document Group By role")
...
def invalidGroupByClauseShouldThrow():
    ...
    try:
        result = executeStatement("Select name, count(*) From document Group By role")
...
def staticColumnAndFunction():
    result = executeStatement("Select 'Sum', Sum(num) From document")

    assert result == [["Sum", 21]]


def emptyResultShouldNotThrowOnFunction():
    result = executeStatement("Select count(*) From document where role = 'xxx'")

    assert result == []


def concatFunction():
    result = executeStatement("Select concat(tag, ' ', role) From document")
...
def typeFunction():
    result = executeStatement("Select type(*) From document")

    assert result == [
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.DocumentObject"],
        ["test.sql_parser_test.ProxyObject"],
    ]


def renamedColumnNamesShouldBeCorrect():
    result, statement = executeStatement(
        "Select name As 'Something', 42 As 'Number','literal' From document",
...
def simpleCalculation():
    result = executeStatement("Select 4 * 2, 8 + 1, 6 / 2, 5 - 3 From document")
...
def referenceCalculation():
    result = executeStatement("Select num * 2 From document")
